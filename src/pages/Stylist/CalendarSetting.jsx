...
import { BUSINESS_TIME_SLOTS, filterBusinessTimeSlots } from '../../lib/supabase';
...
function StylistCalendarSetting({ scheduleDatabase = {}, setScheduleDatabase = () => {}, onSaveSchedule = async () => {} }) {
  // 2026年專題基準時間點
  ...
  const currentDayTimes = filterBusinessTimeSlots(scheduleDatabase[selectedDate] || []);
  ...
  const hasSchedule = (dateString) => {
    return filterBusinessTimeSlots(scheduleDatabase[dateString] || []).length > 0;
  };
  ...
  return (
    <div style={{ padding: '16px', fontFamily: 'sans-serif', background: 'transparent' }}>
      
      {/* 1. 頂部標題 */}
      <div style={{ marginBottom: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        ...
        <button
          ...
          style={{
            ...
            border: 'none',
            borderRadius: '999px',
            background: isSaving ? '#9b7779' : colors.primary,
            color: '#fff',
            fontSize: '14px',
            fontWeight: 'bold',
            cursor: isSaving ? 'not-allowed' : 'pointer',
            boxShadow: '0 4px 14px rgba(86,10,12,0.16)',
            whiteSpace: 'nowrap'
          }}
        >
          {isSaving ? '儲存中...' : '儲存班表'}
      ...
      {/* 2. 三個月滾動日曆卡片 */}
      <div style={{ background: 'rgba(255, 248, 245, 0.58)', border: `1px solid ${colors.accent}`, borderRadius: '16px', padding: '16px', boxShadow: '0 12px 30px rgba(86,10,12,0.08)', marginBottom: '20px' }}>
